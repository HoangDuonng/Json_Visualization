...
import { getContainingFrame } from "@jsondraw/element";

import type { ValueOf } from "@jsondraw/common/utility-types";

import type { IMAGE_MIME_TYPES, STRING_MIME_TYPES } from "@jsondraw/common";
import type {
  JsonDrawElement,
  NonDeletedJsonDrawElement,
} from "@jsondraw/element/types";

import { JsonDrawError } from "./errors";
import {
  createFile,
  getFileHandle,
...
type ElementsClipboard = {
  type: typeof EXPORT_DATA_TYPES.jsondrawClipboard;
...
const clipboardContainsElements = (
  contents: any,
): contents is { elements: JsonDrawElement[]; files?: BinaryFiles } => {
  if (
    [
      EXPORT_DATA_TYPES.jsondraw,
      EXPORT_DATA_TYPES.jsondrawClipboard,
      EXPORT_DATA_TYPES.jsondrawClipboardWithAPI,
...
export const serializeAsClipboardJSON = ({
  ...
  const contents: ElementsClipboard = {
    type: EXPORT_DATA_TYPES.jsondrawClipboard,
...
export const copyToClipboard = async (
  ...
  await copyTextToSystemClipboard(
    {
      [MIME_TYPES.jsondrawClipboard]: json,
    ...
    for (const type of item.types) {
      ...
      try {
        if (type === MIME_TYPES.text || type === MIME_TYPES.html) {
          ...
          throw new JsonDrawError(`Unsupported clipboard type: ${type}`);
        }
      } catch (error: any) {
        console.warn(
          error instanceof JsonDrawError
            ? error.message
...
export const parseClipboard = async (
  ...
  try {
    ...
    const programmaticAPI =
      systemClipboardData.type === EXPORT_DATA_TYPES.jsondrawClipboardWithAPI;
