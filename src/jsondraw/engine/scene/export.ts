...
const truncateText = (element: JsonDrawTextElement, maxWidth: number) => {
...
const addFrameLabelsAsTextElements = (
  ...
  const nextElements: NonDeletedJsonDrawElement[] = [];
  for (const element of elements) {
    if (isFrameLikeElement(element)) {
      let textElement: Mutable<JsonDrawTextElement> = newTextElement({
        x: element.x,
        y: element.y - FRAME_STYLE.nameOffsetY,
        fontFamily: FONT_FAMILY.Helvetica,
        fontSize: FRAME_STYLE.nameFontSize,
        lineHeight:
          FRAME_STYLE.nameLineHeight as JsonDrawTextElement["lineHeight"],
...
const prepareElementsForRender = ({
  ...
  exportingFrame: JsonDrawFrameLikeElement | null | undefined;
  frameRendering: AppState["frameRendering"];
  exportWithDarkMode: AppState["exportWithDarkMode"];
}) => {
  let nextElements: readonly JsonDrawElement[];
...
export const exportToSvg = async (
  ...
  svgRoot.appendChild(createHTMLComment("svg-source:jsondraw"));
...
export const encodeSvgBase64Payload = ({
  ...
  metadataElement.appendChild(
    createHTMLComment(`payload-type:${MIME_TYPES.jsondraw}`),
  );
...
export const decodeSvgBase64Payload = ({ svg }: { svg: string }) => {
  if (svg.includes(`payload-type:${MIME_TYPES.jsondraw}`)) {
