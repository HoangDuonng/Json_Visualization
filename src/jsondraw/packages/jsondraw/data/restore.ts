import { isFiniteNumber, pointFrom } from "@jsondraw/math";
...
import { normalizeFixedPoint } from "@jsondraw/element";
import {
...
import { detectLineHeight } from "@jsondraw/element";
import {
...
import { syncInvalidIndices } from "@jsondraw/element";

import { refreshTextDimensions } from "@jsondraw/element";

import { getNormalizedDimensions } from "@jsondraw/element";
...
import type { MarkOptional, Mutable } from "@jsondraw/common/utility-types";
...
export const AllowedJsonDrawActiveTools: Record<
...
export type RestoredDataState = {
  elements: OrderedJsonDrawElement[];
...
const repairBinding = <T extends JsonDrawArrowElement>(
  element: T,
  ...
  try {
    ...
    if (isElbowArrow(element)) {
      const fixedPointBinding:
        | JsonDrawElbowArrowElement["startBinding"]
        | JsonDrawElbowArrowElement["endBinding"] = {
        ...binding,
    ...
    const targetBoundElement =
      (targetElementsMap.get(binding.elementId) as JsonDrawBindableElement) ||
      undefined;
    const boundElement =
      targetBoundElement ||
...
const restoreElementWithProperties = <
  T extends Required<Omit<JsonDrawElement, "customData">> & {
    customData?: JsonDrawElement["customData"];
    /** @deprecated */
    boundElementIds?: readonly JsonDrawElement["id"][];
    /** @deprecated */
    strokeSharpness?: StrokeRoundness;
  },
  K extends Pick<T, keyof Omit<Required<T>, keyof JsonDrawElement>>,
>(
  element: T,
  extra: Pick<
    T,
    ...
    keyof K
  > &
    Partial<Pick<JsonDrawElement, "type" | "x" | "y" | "customData">>,
): T => {
  const base: Pick<T, keyof JsonDrawElement> = {
...
export const restoreElement = (
  ...
  element: Exclude<JsonDrawElement, JsonDrawSelectionElement>,
  ...
  switch (element.type) {
    case "text":
      // temp fix: cleanup legacy obsidian-jsondraw attribute else it'll
    ...
    case "arrow": {
      ...
      const base = {
        ...
        x,
        y,
        elbowed: (element as JsonDrawArrowElement).elbowed,
        ...getSizeFromPoints(points),
      };

      // TODO: Separate arrow from linear element
      const restoredElement = isElbowArrow(element)
        ? restoreElementWithProperties(element as JsonDrawElbowArrowElement, {
            ...base,
            elbowed: true,
            fixedSegments:
        ...
        : restoreElementWithProperties(element as JsonDrawArrowElement, base);
...
const repairContainerElement = (
  container: Mutable<JsonDrawElement>,
  elementsMap: Map<string, Mutable<JsonDrawElement>>,
) => {
  if (container.boundElements) {
    ...
    const boundIds = new Set<JsonDrawElement["id"]>();
    container.boundElements = boundElements.reduce(
      (
        acc: Mutable<NonNullable<JsonDrawElement["boundElements"]>>,
        binding,
      ) => {
...
const repairBoundElement = (
  boundElement: Mutable<JsonDrawTextElement>,
  elementsMap: Map<string, Mutable<JsonDrawElement>>,
) => {
...
const repairFrameMembership = (
  element: Mutable<JsonDrawElement>,
  elementsMap: Map<string, Mutable<JsonDrawElement>>,
) => {
  if (element.frameId) {
...
export const restoreElements = <T extends JsonDrawElement>(
  ...
  opts?:
    ...
    | undefined,
): CombineBrandsIfNeeded<T, OrderedJsonDrawElement> => {
  ...
  const restoredElements = syncInvalidIndices(
    (targetElements || []).reduce((elements, element) => {
      ...
      let migratedElement: JsonDrawElement | null;
      try {
  ...
  for (const element of restoredElements) {
    ...
    if (isLinearElement(element)) {
      if (
        ...
        (element as Mutable<JsonDrawLinearElement>).startBinding = null;
      }
      if (
        element.endBinding &&
        ...
        (element as Mutable<JsonDrawLinearElement>).endBinding = null;
      }
    }
  }
  ...
  return restoredElements.map((element) => {
    ...
    return element;
  }) as CombineBrandsIfNeeded<T, OrderedJsonDrawElement>;
};
 ...
 *
 * See https://github.com/jsondraw/jsondraw/issues/3795
 *
 ...
 * updates).
 */
export const bumpElementVersions = <T extends JsonDrawElement>(
    ...
    activeTool: {
      ...updateActiveTool(
        ...
        nextAppState.activeTool.type &&
          AllowedJsonDrawActiveTools[nextAppState.activeTool.type]
