...
import type { JsonDrawElement, FileId } from "@jsondraw/element/types";
...
import { isValidJsonDrawData, isValidLibrary } from "./json";
import {
  restoreAppState,
  restoreElements,
  restoreLibraryItems,
...
export const getMimeType = (blob: Blob | string): string => {
  ...
  } else if (/\.jsondrawlib$/.test(name)) {
    return MIME_TYPES.jsondrawlib;
  }
  return "";
};
...
export const getFileHandleType = (handle: FileSystemHandle | null) => {
  ...
  return handle.name.match(/\.(json|jsondraw|png|svg)$/)?.[1] || null;
};
 ...
 * Note: doesn't handle missing .jsondraw/.jsondrawlib extension  */
export const normalizeFile = async (file: File) => {
  ...
  if (file?.name?.endsWith(".jsondrawlib")) {
    file = createFile(file, MIME_TYPES.jsondrawlib, file.name);
  } else if (file?.name?.endsWith(".jsondraw")) {
    file = createFile(file, MIME_TYPES.jsondraw, file.name);
