...
import type { LinearElementEditor } from "@jsondraw/element";
...
import type { GlobalPoint } from "@jsondraw/math";
...
export type BinaryFiles = Record<JsonDrawElement["id"], BinaryFileData>;
...
export type ElementOrToolType = JsonDrawElementType | ToolType | "custom";
...
export interface AppState {
   ...
   */
  multiElement: NonDeleted<JsonDrawLinearElement> | null;
  /**
   * decoupled from newElement, dragging selection only creates selectionElement
   * - set on pointer down, updated during pointer move
   */
  selectionElement: NonDeletedJsonDrawElement | null;
  isBindingEnabled: boolean;
  startBoundElement: NonDeleted<JsonDrawBindableElement> | null;
  suggestedBinding: {
    element: NonDeleted<JsonDrawBindableElement>;
    midPoint?: GlobalPoint;
  } | null;
  frameToHighlight: NonDeleted<JsonDrawFrameLikeElement> | null;
  frameRendering: {
    enabled: boolean;
    name: boolean;
    outline: boolean;
  ...
  elementsToHighlight: NonDeleted<JsonDrawElement>[] | null;
  /**
   * set when a new text is created or when an existing text is being edited
   */
  editingTextElement: NonDeletedJsonDrawElement | null;
  activeTool: {
    /**
  ...
  currentItemFillStyle: JsonDrawElement["fillStyle"];
  currentItemStrokeWidth: number;
  currentItemStrokeStyle: JsonDrawElement["strokeStyle"];
  ...
  /** image cropping */
  isCropping: boolean;
  croppingElementId: JsonDrawElement["id"] | null;

  /** null if no search matches found / search closed */
  searchMatches: Readonly<{
    focusedId: JsonDrawElement["id"] | null;
...
export type LibraryItem_v1 = readonly NonDeleted<JsonDrawElement>[];
/** @deprecated legacy: do not use outside of migration paths */
...
export type JsonDrawInitialDataState = Merge<
  ImportedDataState,
  {
...
export interface JsonDrawProps {
  onChange?: (
    elements: readonly OrderedJsonDrawElement[],
    appState: AppState,
    files: BinaryFiles,
  ) => void;
  onIncrement?: (event: DurableIncrement | EphemeralIncrement) => void;
  initialData?:
    | (() => MaybePromise<JsonDrawInitialDataState | null>)
    | MaybePromise<JsonDrawInitialDataState | null>;
  jsondrawAPI?: (api: JsonDrawImperativeAPI) => void;
   ...
   */
  onDuplicate?: (
    nextElements: readonly JsonDrawElement[],
    /** excludes the duplicated elements */
    prevElements: readonly JsonDrawElement[],
  ) => JsonDrawElement[] | void;
  renderTopLeftUI?: (
    isMobile: boolean,
    appState: UIAppState,
  ) => JSX.Element | null;
  ...
  renderEmbeddable?: (
    element: NonDeleted<JsonDrawEmbeddableElement>,
    appState: AppState,
  ) => JSX.Element | null;
  aiEnabled?: boolean;
...
export type ExportOpts = {
  ...
  onExportToBackend?: (
    exportedElements: readonly NonDeletedJsonDrawElement[],
    appState: UIAppState,
    files: BinaryFiles,
  ) => void;
  renderCustomUI?: (
...
export type AppClassProperties = {
  ...
  jsondrawContainerValue: App["jsondrawContainerValue"];
...
export type PointerDownState = Readonly<{
  ...
  originalElements: Map<string, NonDeleted<JsonDrawElement>>;
  resize: {
  ...
  hit: {
    ...
    // pointer down event
    element: NonDeleted<JsonDrawElement> | null;
...
export interface JsonDrawImperativeAPI {
...
export type FrameNameBoundsCache = {
  get: (
    frameElement: JsonDrawFrameLikeElement | JsonDrawMagicFrameElement,
  ) => FrameNameBounds | null;
  _cache: Map<
    string,
    FrameNameBounds & {
      zoom: AppState["zoom"]["value"];
      versionNonce: JsonDrawFrameLikeElement["versionNonce"];
    }
  >;
};
...
export type EmbedsValidationStatus = Map<
  JsonDrawIframeLikeElement["id"],
  boolean
>;

export type ElementsPendingErasure = Set<JsonDrawElement["id"]>;

export type PendingJsonDrawElements = JsonDrawElement[];
...
export type GenerateDiagramToCode = (props: {
  frame: JsonDrawMagicFrameElement;
  children: readonly JsonDrawElement[];
