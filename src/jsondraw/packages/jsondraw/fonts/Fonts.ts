...
import { charWidth } from "@jsondraw/element";
import { containsCJK } from "@jsondraw/element";
import { ShapeCache } from "@jsondraw/element";
import { isTextElement } from "@jsondraw/element";
import type { Scene } from "@jsondraw/element";
import type { JsonDrawElement, JsonDrawTextElement } from "@jsondraw/element/types";
...
import { JsonDrawFontFace } from "./JsonDrawFontFace";
...
export class Fonts {
   ...
   */
  public static async generateFontFaceDeclarations(elements: readonly JsonDrawElement[]) {
  ...
  private static async loadFontFaces(
    fontFamilies: Array<JsonDrawTextElement["fontFamily"]>,
  ...
  private static register(
    this:
      | Fonts
      | {
          registered: Map<number, { metadata: FontMetadata; fontFaces: JsonDrawFontFace[] }>;
        },
    family: string,
    metadata: FontMetadata,
    ...fontFacesDecriptors: JsonDrawFontFaceDescriptor[]
  ) {
    ...
    if (!registeredFamily) {
      this.registered.set(fontFamily, {
        ...
        fontFaces: fontFacesDecriptors.map(
          ({ uri, descriptors }) => new JsonDrawFontFace(family, uri, descriptors)
        ),
      });
    }
  ...
  private static init() {
    const fonts = {
      registered: new Map<
        ...
        { metadata: FontMetadata; fontFaces: JsonDrawFontFace[] }
      >(),
    };

    const init = (
      family: keyof typeof FONT_FAMILY | keyof typeof FONT_FAMILY_FALLBACKS,
      ...fontFacesDescriptors: JsonDrawFontFaceDescriptor[]
    ) => {
      const fontFamily =
        FONT_FAMILY[family as keyof typeof FONT_FAMILY] ??
        FONT_FAMILY_FALLBACKS[family as keyof typeof FONT_FAMILY_FALLBACKS];
  ...
  private static getUniqueFamilies(
    elements: ReadonlyArray<JsonDrawElement>
  ): Array<JsonDrawTextElement["fontFamily"]> {
    return Array.from(
...
export interface JsonDrawFontFaceDescriptor {
  uri: string;
