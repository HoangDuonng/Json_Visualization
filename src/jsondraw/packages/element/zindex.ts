import { arrayToMap, findIndex, findLastIndex } from "@jsondraw/common";
...
const isOfTargetFrame = (element: JsonDrawElement, frameId: string) => {
...
const getIndicesToMove = (
  ...
  appState: AppState,
  elementsToBeMoved?: readonly JsonDrawElement[],
) => {
...
const getContiguousFrameRangeElements = (
  allElements: readonly JsonDrawElement[],
  frameId: JsonDrawFrameLikeElement["id"],
) => {
  let rangeStart = -1;
  let rangeEnd = -1;
...
export const moveArrowAboveBindable = (
  ...
  scene: Scene,
  hit?: NonDeletedJsonDrawElement,
): readonly OrderedJsonDrawElement[] => {
  ...
  const bindableIds = [
    ...
    boundTextElement?.id,
    containerElement?.id,
  ].filter((id): id is NonDeletedJsonDrawElement["id"] => !!id);
...
const getTargetIndex = (
   ...
   */
  containingFrame: JsonDrawFrameLikeElement["id"] | null,
  scene: Scene,
) => {
  const sourceElement = elements[boundaryIndex];

  const indexFilter = (element: JsonDrawElement) => {
...
const getTargetElementsMap = <T extends JsonDrawElement>(
  elements: readonly T[],
  indices: number[],
) => {
  return indices.reduce((acc, index) => {
    ...
    return acc;
  }, new Map<string, JsonDrawElement>());
};
...
const shiftElementsByOne = (
  ...
  const selectedFrames = new Set<JsonDrawFrameLikeElement["id"]>(
    indicesToMove
...
const shiftElementsToEnd = (
  ...
  const targetElementsMap = getTargetElementsMap(elements, indicesToMove);
  const displacedElements: JsonDrawElement[] = [];
...
function shiftElementsAccountingForFrames(
  ...
  shiftFunction: (
    ...
    containingFrame: JsonDrawFrameLikeElement["id"] | null,
    elementsToBeMoved?: readonly JsonDrawElement[],
  ) => JsonDrawElement[] | readonly JsonDrawElement[],
) {
  ...
  const frameAwareContiguousElementsToMove: {
    regularElements: JsonDrawElement[];
    frameChildren: Map<JsonDrawFrameLikeElement["id"], JsonDrawElement[]>;
  } = { regularElements: [], frameChildren: new Map() };

  const fullySelectedFrames = new Set<JsonDrawFrameLikeElement["id"]>();
