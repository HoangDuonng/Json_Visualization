...
import type { Curve, LineSegment, LocalPoint } from "@jsondraw/math";
...
const ElementShapesCache = new WeakMap<
  JsonDrawElement,
  { version: JsonDrawElement["version"]; shapes: Map<number, ElementShape> }
>();

const getElementShapesCacheEntry = <T extends JsonDrawElement>(
  element: T,
  offset: number,
...
const setElementShapesCacheEntry = <T extends JsonDrawElement>(
  element: T,
  shape: ElementShape,
  offset: number,
) => {
...
export const getCornerRadius = (x: number, element: JsonDrawElement) => {
  if (
