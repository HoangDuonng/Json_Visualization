 ...
 */
export const duplicateElement = <TElement extends JsonDrawElement>(
...
export const duplicateElements = (
  opts: {
    ...
    randomizeSeed?: boolean;
    overrides?: (data: {
      duplicateElement: JsonDrawElement;
      origElement: JsonDrawElement;
      origIdToDuplicateId: Map<
        JsonDrawElement["id"],
        JsonDrawElement["id"]
      >;
    }) => Partial<JsonDrawElement>;
  } & (
    | {
        /**
  ...
  // loop over them.
  const processedIds = new Map<JsonDrawElement["id"], true>();
  const groupIdMap = new Map();
  const duplicatedElements: JsonDrawElement[] = [];
  const origElements: JsonDrawElement[] = [];
  ...
  const duplicateIdToOrigElement = new Map<
    ...
    JsonDrawElement
  >();
  const duplicateElementsMap = new Map<string, JsonDrawElement>();
  ...
  const elementsWithDuplicates: JsonDrawElement[] = elements.slice();
  ...
  const copyElements = <T extends JsonDrawElement | JsonDrawElement[]>(
    element: T,
  ): T extends JsonDrawElement[]
    ? JsonDrawElement[]
    : JsonDrawElement | null => {
    const elements = castArray(element);

    const _newElements = elements.reduce(
      (acc: JsonDrawElement[], element) => {
  ...
  const insertBeforeOrAfterIndex = (
    index: number,
    elements: JsonDrawElement | null | JsonDrawElement[],
  ) => {
    if (!elements) {
      return;
    }
...
// Simplified deep clone for the purpose of cloning JsonDrawElement.
//
...
// passing JsonDrawElement as the top-level argument).
const _deepCopyElement = (val: any, depth: number = 0) => {
  ...
  // don't support them on jsondraw elements yet. If we do, we need to make
...
/**
 * Clones JsonDrawElement data structure. Does not regenerate id, nonce, or
 ...
 * Typed arrays and other non-null objects.
 */
export const deepCopyElement = <T extends JsonDrawElement>(
  val: T,
): Mutable<T> => {
