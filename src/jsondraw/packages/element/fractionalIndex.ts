 ...
 * - it's necessary to always keep the array support for backwards compatibility (restore) - old scenes, old libraries, supporting multiple jsondraw versions etc.
...
export const validateFractionalIndices = (
  elements: readonly JsonDrawElement[],
  {
    ...
    ignoreLogs?: true;
    reconciliationContext?: {
      localElements: ReadonlyArray<JsonDrawElement>;
      remoteElements: ReadonlyArray<JsonDrawElement>;
    };
  }
) => {
  const errorMessages: string[] = [];
  const stringifyElement = (element: JsonDrawElement | void) =>
  ...
  if (errorMessages.length) {
    ...
    const additionalContext: (string | string[])[] = [];
 ...
 */
export const orderByFractionalIndex = (elements: OrderedJsonDrawElement[]) => {
...
export const syncMovedIndices = (
  ...
  return elements as OrderedJsonDrawElement[];
};
 ...
 */
const getInvalidIndicesGroups = (elements: readonly JsonDrawElement[]) => {
  const indicesGroups: number[][] = [];

  // once we find lowerBound / upperBound, it cannot be lower than that, so we cache it for better perf.
  let lowerBound: JsonDrawElement["index"] | undefined = undefined;
  let upperBound: JsonDrawElement["index"] | undefined = undefined;
  let lowerBoundIndex: number = -1;
  let upperBoundIndex: number = 0;

  /** @returns maybe valid lowerBound */
  const getLowerBound = (index: number): [JsonDrawElement["index"] | undefined, number] => {
  ...
  const getUpperBound = (index: number): [JsonDrawElement["index"] | undefined, number] => {
...
const isValidFractionalIndex = (
  index: JsonDrawElement["index"] | undefined,
  predecessor: JsonDrawElement["index"] | undefined,
  successor: JsonDrawElement["index"] | undefined
) => {
  if (!index) {
    return false;
  }
...
const generateIndices = (elements: readonly JsonDrawElement[], indicesGroups: number[][]) => {
  const elementsUpdates = new Map<JsonDrawElement, { index: FractionalIndex }>();
...
const isOrderedElement = (element: JsonDrawElement): element is OrderedJsonDrawElement => {
