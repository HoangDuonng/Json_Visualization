import { KEYS, arrayToMap, getFeatureFlag, invariant, isTransparent } from "@jsondraw/common";
import type { Bounds } from "@jsondraw/common";
import type { MapEntry, Mutable } from "@jsondraw/common/utility-types";
...
import type { LineSegment, LocalPoint, Radians } from "@jsondraw/math";
...
export type BindingStrategy =
  ...
  | {
      mode: BindMode;
      element: NonDeleted<JsonDrawBindableElement>;
...
export const getBindingGap = (
  bindTarget: JsonDrawBindableElement,
  opts: Pick<JsonDrawArrowElement, "elbowed">
): number => {
...
const bindingStrategyForElbowArrowEndpointDragging = (
  ...
  elements: readonly Ordered<NonDeletedJsonDrawElement>[],
  zoom?: AppState["zoom"]
): {
...
const bindingStrategyForSimpleArrowEndpointDragging_complex = (
  ...
  const oppositeElement = oppositeBinding
    ? (elementsMap.get(oppositeBinding.elementId) as JsonDrawBindableElement)
    : null;
...
const getBindingStrategyForDraggingBindingElementEndpoints_simple = (
  ...
  const otherBindableElement = otherBinding
    ? (elementsMap.get(otherBinding.elementId) as NonDeleted<JsonDrawBindableElement>)
    : undefined;
...
export const bindOrUnbindBindingElements = (
  selectedArrows: NonDeleted<JsonDrawArrowElement>[],
  scene: Scene,
  appState: AppState
): void => {
...
export const bindBindingElement = (
  arrow: NonDeleted<JsonDrawArrowElement>,
  hoveredElement: JsonDrawBindableElement,
  mode: BindMode,
  startOrEnd: "start" | "end",
  scene: Scene,
  focusPoint?: GlobalPoint
...
export const unbindBindingElement = (
  ...
  startOrEnd: "start" | "end",
  scene: Scene
): JsonDrawBindableElement["id"] | null => {
  ...
  if (!oppositeBinding || oppositeBinding.elementId !== binding.elementId) {
    ...
    const boundElement = scene
      ...
      .get(binding.elementId) as JsonDrawBindableElement;
...
export const updateBoundElements = (
  changedElement: NonDeletedJsonDrawElement,
  scene: Scene,
  options?: {
    simultaneouslyUpdated?: readonly JsonDrawElement[];
    changedElements?: Map<string, JsonDrawElement>;
  ...
  const visitor = (element: JsonDrawElement | undefined) => {
...
const updateArrowBindings = (
  latestElement: JsonDrawArrowElement,
  ...
  const bindableElement =
    binding && (elementsMap.get(binding.elementId) as JsonDrawBindableElement);
...
const doesNeedUpdate = (
  boundElement: NonDeleted<JsonDrawArrowElement>,
  changedElement: JsonDrawBindableElement
) => {
  return (
...
const getSimultaneouslyUpdatedElementIds = (
  simultaneouslyUpdated: readonly JsonDrawElement[] | undefined
): Set<JsonDrawElement["id"]> => {
...
export const getHeadingForElbowArrowSnap = (
  ...
  bindableElement: JsonDrawBindableElement | undefined | null,
...
const getDistanceForBinding = (
  ...
  bindableElement: JsonDrawBindableElement,
  elementsMap: ElementsMap,
  zoom?: AppState["zoom"]
) => {
...
export const bindPointToSnapToElementOutline = (
  arrowElement: JsonDrawArrowElement,
  bindableElement: JsonDrawBindableElement,
  startOrEnd: "start" | "end",
  elementsMap: ElementsMap,
  ...
  if (elbowed) {
    ...
    const snapPoint = snapToMid(bindableElement, elementsMap, edgePoint, 0.05, arrowElement);
...
const compareElementArea = (a: JsonDrawBindableElement, b: JsonDrawBindableElement) =>
...
export const updateBoundPoint = (
  ...
  const otherBindableElement =
    otherBinding && (elementsMap.get(otherBinding.elementId)! as JsonDrawBindableElement);
...
export const calculateFixedPointForElbowArrowBinding = (
  linearElement: NonDeleted<JsonDrawElbowArrowElement>,
  hoveredElement: JsonDrawBindableElement,
  startOrEnd: "start" | "end",
  elementsMap: ElementsMap
...
export const calculateFixedPointForNonElbowArrowBinding = (
  linearElement: NonDeleted<JsonDrawArrowElement>,
  hoveredElement: JsonDrawBindableElement,
  startOrEnd: "start" | "end",
  elementsMap: ElementsMap,
  focusPoint?: GlobalPoint
...
export const fixDuplicatedBindingsAfterDuplication = (
  duplicatedElements: JsonDrawElement[],
  origIdToDuplicateId: Map<JsonDrawElement["id"], JsonDrawElement["id"]>,
    ...
    if ("boundElements" in duplicateElement && duplicateElement.boundElements) {
      Object.assign(duplicateElement, {
        boundElements: duplicateElement.boundElements.reduce(
          (acc: Mutable<NonNullable<JsonDrawElement["boundElements"]>>, binding) => {
...
export const fixBindingsAfterDeletion = (
  sceneElements: readonly JsonDrawElement[],
  deletedElements: readonly JsonDrawElement[]
): void => {
...
const newBoundElements = (
  boundElements: JsonDrawElement["boundElements"],
  idsToRemove: Set<JsonDrawElement["id"]>,
  elementsToAdd: Array<JsonDrawElement> = []
) => {
  if (!boundElements) {
    return null;
  }
  ...
  nextBoundElements.push(
    ...elementsToAdd.map(
      x => ({ id: x.id, type: x.type }) as JsonDrawArrowElement | JsonDrawTextElement
    )
  );
...
type BoundElementsVisitingFunc = (
  boundElement: JsonDrawElement | undefined,
  bindingProp: BindableProp,
  bindingId: string
) => void;

type BindableElementVisitingFunc<T> = (
  bindableElement: JsonDrawElement | undefined,
...
export class BoundElement {
  ...
  public static unbindAffected(
    ...
    boundElement: JsonDrawElement | undefined,
    updateElementWith: (
      affected: JsonDrawElement,
      updates: ElementUpdate<JsonDrawElement>
    ) => void
  ) {
    if (!boundElement) {
      return;
      ...
      if (isTextElement(boundElement)) {
        ...
        if (boundElements.reverse().find(x => x.type === "text")?.id === boundElement.id) {
          if (boundElement.containerId !== bindableElement.id) {
            ...
            } as ElementUpdate<JsonDrawTextElement>);
          }
        } else {
...
export const getGlobalFixedPointForBindableElement = (
  ...
  const normalized = normalizeFixedPoint(fixedPointRatio);
  const [fixedX, fixedY] = normalized ?? [0.5, 0.5];
...
export const getGlobalFixedPoints = (
  ...
  const startElement =
    arrow.startBinding &&
    (elementsMap.get(arrow.startBinding.elementId) as JsonDrawBindableElement | undefined);
  const endElement =
    arrow.endBinding &&
    (elementsMap.get(arrow.endBinding.elementId) as JsonDrawBindableElement | undefined);
  const startPoint =
    startElement && arrow.startBinding
      ? getGlobalFixedPointForBindableElement(
          arrow.startBinding.fixedPoint,
          startElement as JsonDrawBindableElement,
          elementsMap
        )
  ...
  const endPoint =
    endElement && arrow.endBinding
      ? getGlobalFixedPointForBindableElement(
          arrow.endBinding.fixedPoint,
          endElement as JsonDrawBindableElement,
          elementsMap
        )
      : pointFrom<GlobalPoint>(
...
export const getArrowLocalFixedPoints = (
  arrow: JsonDrawElbowArrowElement,
  elementsMap: ElementsMap
) => {
...
const getShapeType = (element: JsonDrawBindableElement): ShapeType => {
...
export const getBindingSideMidPoint = (binding: FixedPointBinding, elementsMap: ElementsMap) => {
  ...
  if (isRectangularElement(bindableElement)) {
    const [sides, corners] = deconstructRectanguloidElement(
      bindableElement as JsonDrawRectanguloidElement
    );
