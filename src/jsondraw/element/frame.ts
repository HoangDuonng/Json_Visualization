import { arrayToMap } from "@jsondraw/common";
import { isPointWithinBounds, pointFrom } from "@jsondraw/math";
import { doLineSegmentsIntersect } from "@jsondraw/utils/bbox";
import { elementsOverlappingBBox } from "@jsondraw/utils/withinBounds";
...
import type { ReadonlySetLike } from "@jsondraw/common/utility-types";
...
import type { JsonDrawElementsIncludingDeleted } from "./Scene";
...
export const bindElementsToFramesAfterDuplication = (
  ...
  origElements: readonly JsonDrawElement[],
 ...
 */
export const groupByFrameLikes = (elements: readonly JsonDrawElement[]) => {
...
export const getFrameChildren = (
  ...
  const frameChildren: JsonDrawElement[] = [];
...
export const getFrameLikeElements = (
  allElements: JsonDrawElementsIncludingDeleted,
): JsonDrawFrameLikeElement[] => {
  return allElements.filter((element): element is JsonDrawFrameLikeElement =>
    isFrameLikeElement(element),
  );
};

/**
 * Returns JsonDrawFrameElements and non-frame-children elements.
 *
...
export const getElementsInResizingFrame = (
  ...
  const nextElementsInFrame = new Set<JsonDrawElement>(prevElementsInFrame);
...
export const getElementsInNewFrame = (
  elements: JsonDrawElementsIncludingDeleted,
  frame: JsonDrawFrameLikeElement,
  elementsMap: ElementsMap,
) => {
...
export const getContainingFrame = (
  ...
  return (elementsMap.get(element.frameId) ||
    null) as null | JsonDrawFrameLikeElement;
};
...
export const filterElementsEligibleAsFrameChildren = (
  ...
  const otherFrames = new Set<JsonDrawFrameLikeElement["id"]>();
  const elementsMap = arrayToMap(elements);
  ...
  const processedGroups = new Set<JsonDrawElement["id"]>();

  const eligibleElements: JsonDrawElement[] = [];
...
export const addElementsToFrame = <T extends ElementsMapOrArray>(
  allElements: T,
  elementsToAdd: NonDeletedJsonDrawElement[],
  frame: JsonDrawFrameLikeElement,
  appState: AppState,
): T => {
  const elementsMap = arrayToMap(allElements);
  const currTargetFrameChildrenMap = new Map<JsonDrawElement["id"], true>();
  for (const element of allElements.values()) {
  ...
  const finalElementsToAdd: JsonDrawElement[] = [];
...
export const removeElementsFromFrame = (
  elementsToRemove: ReadonlySetLike<NonDeletedJsonDrawElement>,
  elementsMap: ElementsMap,
) => {
...
export const removeAllElementsFromFrame = <T extends JsonDrawElement>(
...
export const replaceAllElementsInFrame = <T extends JsonDrawElement>(
  allElements: readonly T[],
  nextElementsInFrame: JsonDrawElement[],
  frame: JsonDrawFrameLikeElement,
...
>(
  ...
  const elementsToFilter = new Set<JsonDrawElement>(selectedElements);
  ...
  const elementsToRemove = new Set<JsonDrawElement>();
...
export const isElementInFrame = (
  ...
  appState: StaticCanvasAppState,
  opts?: {
    targetFrame?: JsonDrawFrameLikeElement;
...
export const getDefaultFrameName = (element: JsonDrawFrameLikeElement) => {
  // TODO name frames "AI" only if specific to AI frames
  return isFrameElement(element) ? DEFAULT_FRAME_NAME : DEFAULT_AI_FRAME_NAME;
};

export const getFrameLikeTitle = (element: JsonDrawFrameLikeElement) => {
