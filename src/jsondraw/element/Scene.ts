...
// in our codebase
export type JsonDrawElementsIncludingDeleted = readonly JsonDrawElement[];
...
export class Scene {
  ...
  private selectedElementsCache: {
    ...
    elements: readonly NonDeletedJsonDrawElement[] | null;
    cache: Map<SelectionHash, NonDeletedJsonDrawElement[]>;
  } = {
  ...
  getNonDeletedFramesLikes(): readonly NonDeleted<JsonDrawFrameLikeElement>[] {
    return this.nonDeletedFramesLikes;
  }

  getElement<T extends JsonDrawElement>(id: T["id"]): T | null {
   ...
   *
   * Maps all current jsondraw elements, invoking the callback for each
  ...
  replaceAllElements(
    ...
    const nextFrameLikes: JsonDrawFrameLikeElement[] = [];
  ...
  insertElementsAtIndex(elements: JsonDrawElement[], index: number) {
    if (!elements.length) {
      return;
    }
  ...
  insertElements = (elements: JsonDrawElement[]) => {
    if (!elements.length) {
      return;
    }
  ...
  getElementsFromId = (id: string): JsonDrawElement[] => {
  ...
  mutateElement<TElement extends Mutable<JsonDrawElement>>(
    element: TElement,
