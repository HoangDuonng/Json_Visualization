...
export class ElementBounds {
  private static boundsCache = new WeakMap<
    ...
    {
      bounds: Bounds;
      version: JsonDrawElement["version"];
    }
  >();
  ...
  if (isFreeDrawElement(element)) {
    ...
    if (isArrowElement(container)) {
      const { x, y } = LinearElementEditor.getBoundTextElementPosition(
        container,
        element as JsonDrawTextElementWithContainer,
        elementsMap,
      );
      return [
        x,
...
export const getElementLineSegments = (
  ...
  if (shape.type === "polycurve") {
    ...
    return getSegmentsOnEllipse(element as JsonDrawEllipseElement);
  }
...
const _isRectanguloidElement = (
  element: JsonDrawElement,
): element is JsonDrawRectanguloidElement => {
  return (
    element != null &&
...
const getSegmentsOnEllipse = (
  ellipse: JsonDrawEllipseElement,
): LineSegment<GlobalPoint>[] => {
 ...
 * Scene -> Scene coords, but in x1,x2,y1,y2 format.
 *
 * Rectangle here means any rectangular frame, not an jsondraw element.
 */
...
export const getDiamondPoints = (element: JsonDrawElement) => {
...
export const getCommonBoundingBox = (
  elements:
    ...
    | readonly NonDeleted<JsonDrawElement>[],
): BoundingBox => {
  const [minX, minY, maxX, maxY] = getCommonBounds(elements);
  return {
    minX,
...
export const aabbForElement = (
  element: Readonly<JsonDrawElement>,
  elementsMap: ElementsMap,
