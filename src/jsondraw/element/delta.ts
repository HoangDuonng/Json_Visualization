...
type ElementPartial<TElement extends JsonDrawElement = JsonDrawElement> =
...
export class ElementsDelta implements DeltaContainer<SceneElementsMap> {
   ...
   */
  public static calculate<T extends OrderedJsonDrawElement>(
  ...
  public applyLatestChanges(
    ...
    const modifier =
      (
        prevElement: OrderedJsonDrawElement | undefined,
        nextElement: OrderedJsonDrawElement | undefined,
      ) =>
      (partial: ElementPartial, partialType: "deleted" | "inserted") => {
        let element: OrderedJsonDrawElement | undefined;
  ...
  public applyTo(
    ...
    let changedElements: Map<string, OrderedJsonDrawElement>;
  ...
  private static createApplier =
    ...
    (deltas: Record<string, Delta<ElementPartial>>) => {
      ...
      return Object.entries(deltas).reduce((acc, [id, delta]) => {
        ...
        return acc;
      }, new Map<string, OrderedJsonDrawElement>());
    };
      ...
      if (!element) {
        ...
        if (element) {
          ...
          element = newElementWith(
            { id, version: 1 } as OrderedJsonDrawElement,
            {
              ...partial,
            },
          );
  ...
  private static applyDelta(
    element: OrderedJsonDrawElement,
  ...
  private resolveConflicts(
    ...
    const nextAffectedElements = new Map<string, OrderedJsonDrawElement>();
    const updater = (
      ...
      const elementUpdates = updates as ElementUpdate<OrderedJsonDrawElement>;

      let affectedElement: OrderedJsonDrawElement;
  ...
  public static redrawElements(
    nextElements: SceneElementsMap,
    changedElements: Map<string, OrderedJsonDrawElement>,
  ) {
    try {
  ...
  private static redrawTextBoundingBoxes(
    scene: Scene,
    changed: Map<string, OrderedJsonDrawElement>,
  ) {
    const elements = scene.getNonDeletedElementsMap();
    const boxesToRedraw = new Map<
      string,
      { container: OrderedJsonDrawElement; boundText: JsonDrawTextElement }
    >();

    for (const element of changed.values()) {
      if (isBoundToContainer(element)) {
        const { containerId } = element as JsonDrawTextElement;
        const container = containerId ? elements.get(containerId) : undefined;

        if (container) {
          boxesToRedraw.set(container.id, {
            container,
            boundText: element as JsonDrawTextElement,
          });
        }
      }
      ...
      if (hasBoundTextElement(element)) {
        ...
        if (boundText) {
          boxesToRedraw.set(element.id, {
            container: element,
            boundText: boundText as JsonDrawTextElement,
          });
        }
      }
    }
      ...
      const deletedPoints =
        (
          deleted as ElementPartial<
            JsonDrawFreeDrawElement | JsonDrawLinearElement
          >
        ).points ?? [];
  ...
  private static stripIrrelevantProps(
    partial: Partial<OrderedJsonDrawElement>,
  ): ElementPartial {
