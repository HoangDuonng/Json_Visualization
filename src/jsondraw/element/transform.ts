import { pointFrom, type LocalPoint } from "@jsondraw/math";
...
import type { MarkOptional } from "@jsondraw/common/utility-types";
  ...
  label?: {
    ...
    | (
        ...
        | ((
            ...
            | {
                type?: "text";
                id: JsonDrawTextElement["id"];
                text: string;
              }
          ) &
            Partial<JsonDrawTextElement>)
      ) &
        MarkOptional<ElementConstructorOpts, "x" | "y">;
  start?:
    | (
        ...
        MarkOptional<ElementConstructorOpts, "x" | "y">;
} & Partial<JsonDrawLinearElement>;

export type ValidContainer =
  | {
      type: Exclude<JsonDrawGenericElement["type"], "selection">;
      id?: JsonDrawGenericElement["id"];
      label?: {
        text: string;
        fontSize?: number;
...
export type JsonDrawElementSkeleton =
  | Extract<
      Exclude<JsonDrawElement, JsonDrawSelectionElement>,
      JsonDrawIframeLikeElement | JsonDrawFreeDrawElement
    >
  | ({
      type: Extract<JsonDrawLinearElement["type"], "line">;
      x: number;
      y: number;
    } & Partial<JsonDrawLinearElement>)
  | ValidContainer
  ...
  | ({
      type: Extract<JsonDrawImageElement["type"], "image">;
      x: number;
      y: number;
      fileId: FileId;
    } & Partial<JsonDrawImageElement>)
  | ({
      type: "frame";
      children: readonly JsonDrawElement["id"][];
      name?: string;
    } & Partial<JsonDrawFrameElement>)
  | ({
      type: "magicframe";
      children: readonly JsonDrawElement["id"][];
      name?: string;
    } & Partial<JsonDrawMagicFrameElement>);
...
const bindTextToContainer = (
  ...
  const textElement: JsonDrawTextElement = newTextElement({
    x: 0,
    y: 0,
...
const bindLinearElementToElement = (
  linearElement: JsonDrawArrowElement,
  start: ValidLinearElement["start"],
  end: ValidLinearElement["end"],
  elementStore: ElementStore,
  scene: Scene,
): {
  linearElement: JsonDrawLinearElement;
  startBoundElement?: JsonDrawElement;
  endBoundElement?: JsonDrawElement;
} => {
  ...
  if (start) {
    ...
    if (startType) {
      ...
      bindBindingElement(
        linearElement,
        startBoundElement as JsonDrawBindableElement,
        "orbit",
        "start",
        scene,
      );
  ...
  if (end) {
    ...
    if (endType) {
      ...
      bindBindingElement(
        linearElement,
        endBoundElement as JsonDrawBindableElement,
        "orbit",
        "end",
        scene,
      );
...
class ElementStore {
  jsondrawElements = new Map<string, JsonDrawElement>();

  add = (ele?: JsonDrawElement) => {
    if (!ele) {
      return;
    }

    this.jsondrawElements.set(ele.id, ele);
  };

  getElements = () => {
    return syncInvalidIndices(Array.from(this.jsondrawElements.values()));
  };
  ...
  getElement = (id: string) => {
    return this.jsondrawElements.get(id);
  };
}

export const convertToJsonDrawElements = (
  elementsSkeleton: JsonDrawElementSkeleton[] | null,
  ...
  const elementsWithIds = new Map<string, JsonDrawElementSkeleton>();
  const oldToNewElementIdMap = new Map<string, string>();

  // Create individual elements
  for (const element of elements) {
    let jsondrawElement: JsonDrawElement;
    ...
    switch (element.type) {
      ...
      case "line": {
        ...
        jsondrawElement = newLinearElement({
          width,
          height,
      ...
      case "arrow": {
        const width = element.width || DEFAULT_LINEAR_ELEMENT_PROPS.width;
        const height = element.height || DEFAULT_LINEAR_ELEMENT_PROPS.height;
        jsondrawElement = newArrowElement({
          width,
          height,
          endArrowhead: "arrow",
          points: [pointFrom(0, 0), pointFrom(width, height)],
        ...
        Object.assign(
          jsondrawElement,
          getSizeFromPoints(jsondrawElement.points),
        );
        break;
      }
      case "text": {
        ...
        jsondrawElement = newTextElement({
          width: metrics.width,
      ...
      case "image": {
        jsondrawElement = newImageElement({
      ...
      case "frame": {
        jsondrawElement = newFrameElement({
          x: 0,
          y: 0,
          ...element,
        });
      ...
      case "magicframe": {
        jsondrawElement = newMagicFrameElement({
          x: 0,
          y: 0,
          ...element,
        });
    ...
    const existingElement = elementStore.getElement(jsondrawElement.id);
    if (existingElement) {
      console.error(`Duplicate id found for ${jsondrawElement.id}`);
    } else {
      elementStore.add(jsondrawElement);
      elementsWithIds.set(jsondrawElement.id, element);
      if (originalId) {
        oldToNewElementIdMap.set(originalId, jsondrawElement.id);
      }
    }
  }
  ...
  for (const [id, element] of elementsWithIds) {
    const jsondrawElement = elementStore.getElement(id)!;
        ...
        if (element.label?.text) {
          ...
          switch (element.type) {
            case "arrow": {
              ...
              const { linearElement, startBoundElement, endBoundElement } =
                bindLinearElementToElement(
                  jsondrawElement as JsonDrawArrowElement,
                  start,
                  end,
                  elementStore,
                  scene,
  ...
  // Once all the jsondraw elements are created, we can add frames since we
  ...
  for (const [id, element] of elementsWithIds) {
    ...
    if (!frame) {
      throw new Error(`JsonDraw element with id ${id} doesn't exist`);
    }
    const childrenElements: JsonDrawElement[] = [];
