import { KEYS, invariant, toBrandedType } from "@jsondraw/common";

import { type GlobalPoint, pointFrom, type LocalPoint } from "@jsondraw/math";

import type {
  AppState,
  PendingJsonDrawElements,
...
import {
  type ElementsMap,
  type JsonDrawBindableElement,
  type JsonDrawElement,
  type JsonDrawFlowchartNodeElement,
  type NonDeletedSceneElementsMap,
  type Ordered,
  type OrderedJsonDrawElement,
} from "./types";
...
const getNodeRelatives = (
  ...
  node: JsonDrawBindableElement,
  elementsMap: ElementsMap,
  direction: LinkDirection,
) => {
  const items = [...elementsMap.values()].reduce(
    (acc: { relative: JsonDrawBindableElement; heading: Heading }[], el) => {
      let oppositeBinding;
      if (
        isElbowArrow(el) &&
        ...
        invariant(
          ...
          "not an JsonDrawBindableElement",
        );
...
const getOffsets = (
  element: JsonDrawFlowchartNodeElement,
  linkedNodes: JsonDrawElement[],
  direction: LinkDirection,
) => {
...
const addNewNode = (
  ...
  invariant(
    ...
    "not an JsonDrawFlowchartNodeElement",
  );
...
export const addNewNodes = (
  startNode: JsonDrawFlowchartNodeElement,
  appState: AppState,
  direction: LinkDirection,
  scene: Scene,
  ...
  const newNodes: JsonDrawElement[] = [];
...
const createBindingArrow = (
  startBindingElement: JsonDrawFlowchartNodeElement,
  endBindingElement: JsonDrawFlowchartNodeElement,
  direction: LinkDirection,
  appState: AppState,
  scene: Scene,
) => {
  ...
  const changedElements = new Map<string, OrderedJsonDrawElement>();
  changedElements.set(
    startBindingElement.id,
    startBindingElement as OrderedJsonDrawElement,
  );
  changedElements.set(
    endBindingElement.id,
    endBindingElement as OrderedJsonDrawElement,
  );
  changedElements.set(
    bindingArrow.id,
    bindingArrow as OrderedJsonDrawElement,
  );
  ...
  const update = updateElbowArrowPoints(
    ...
    toBrandedType<NonDeletedSceneElementsMap>(
      ...
      ] as [string, Ordered<JsonDrawElement>][]),
    ),
...
export class FlowChartNavigator {
  ...
  private sameLevelNodes: JsonDrawElement[] = [];
  ...
  private visitedNodes: Set<JsonDrawElement["id"]> = new Set();
...
export class FlowChartCreator {
  ...
  pendingNodes: PendingJsonDrawElements | null = null;
