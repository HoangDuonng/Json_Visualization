import { arrayToMapWithIndex } from "@jsondraw/common";

import type { JsonDrawElement } from "./types";

const normalizeGroupElementOrder = (elements: readonly JsonDrawElement[]) => {
  const origElements: JsonDrawElement[] = elements.slice();
  const sortedElements = new Set<JsonDrawElement>();

  const orderInnerGroups = (
    elements: readonly JsonDrawElement[],
  ): JsonDrawElement[] => {
    const firstGroupSig = elements[0]?.groupIds?.join("");
    const aGroup: JsonDrawElement[] = [elements[0]];
    const bGroup: JsonDrawElement[] = [];
...
const normalizeBoundElementsOrder = (
  ...
  const origElements: (JsonDrawElement | null)[] = elements.slice();
  const sortedElements = new Set<JsonDrawElement>();
